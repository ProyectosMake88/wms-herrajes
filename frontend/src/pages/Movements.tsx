    ...
    <div>
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="Registrar Movimiento" maxWidth="max-w-xl">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Stock disponible cuando hay producto seleccionado */}
          {selectedProduct && (
            <div className={`flex items-center justify-between p-3 rounded-xl border ${selectedProduct.isLowStock ? 'bg-amber-50 border-amber-200' : 'bg-blue-50 border-blue-200'}`}>
              <div className="flex items-center gap-3">
                {selectedProduct.imageUrl ? (
                  <img src={selectedProduct.imageUrl} alt="" className="w-10 h-10 rounded-lg object-cover border border-white" />
                ) : (
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">{selectedProduct.sku} · {selectedProduct.category?.name}</p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-xs text-gray-500">Stock disponible</p>
                <p className={`text-xl font-bold ${selectedProduct.isLowStock ? 'text-amber-600' : 'text-blue-600'}`}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input required type="number" min="1"
                max={formData.type === 'EXIT' && selectedProduct ? selectedProduct.currentStock : undefined}
                value={formData.quantity} onChange={(e) => setFormData({ ...formData, quantity: e.target.value })}
                className="w-full px-3 py-2.5 bg-gray-50 rounded-xl border border-gray-200 text-sm focus:ring-2 focus:ring-primary-400 focus:outline-none" />
              {formData.type === 'EXIT' && selectedProduct && formData.quantity && Number(formData.quantity) > selectedProduct.currentStock && (
                <p className="text-xs text-red-500 mt-1 font-medium">Excede el stock disponible ({selectedProduct.currentStock})</p>
              )}
            </div>
            <div>
